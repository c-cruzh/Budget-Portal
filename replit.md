...
- **Voluntarios**: Dedicated roster of unpaid, in-kind volunteer staffing roles (PERSONA slots) moved out of Budget Items. Each role has space, día(s), headcount (HC), horarios, job description, do's/don'ts, and guidelines. Grouped by space with totals for Día 1 / Día 2 / overall HC. Add/edit/delete with auto-save and org-permission gating (C2 LABS canEdit; OPINNO/AURORA360 read-only). Money/goods volunteer items (laptops, badges, food) remain in the budget. Data model reserves future per-slot named-person assignment + fill-status (not yet implemented).
- **Espacios Media**: Each Espacio (ESEN Día 1/Día 2 + every Venue/Lugar) supports uploading photos and videos from the device. Thumbnail grid per space, delete (gated to C2 LABS), and a fullscreen lightbox viewer with prev/next navigation (buttons + arrow keys), a counter, and Escape-to-close. Files persist in object storage; only the `objectPath` reference is stored in the spaces catalog (`SpaceMedia[]` on each `SpaceEntry`). Read-only orgs (OPINNO/AURORA360) can view the gallery + lightbox but not upload/delete.
